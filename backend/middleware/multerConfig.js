...
const storage = multer.diskStorage({
  ...
  filename: (req, file, callback) => {
    // on récupère le nom d'origine du fichier, on élimine les espaces (pour éviter des problèmes côté serveur) qu'on remplace par des underscores
...
// On appelle la méthode multer avec notre objet storage, on utilise la méthode single car il s'agit d'un fichier unique et non d'un groupe de fichier et on précise qu'il s'agit d'un fichier image
